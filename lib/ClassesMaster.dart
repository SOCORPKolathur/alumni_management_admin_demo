...
class ClassesMaster extends StatefulWidget {
  const ClassesMaster({Key? key}) : super(key: key);

  @override
  State<ClassesMaster> createState() => _ClassesMasterState();
}

class _ClassesMasterState extends State<ClassesMaster> {
  ...
  doclength() async {
    ...
    final QuerySnapshot result = await FirebaseFirestore.instance.collection("Classes").get();
                ...
                child: SizedBox(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        StreamBuilder(
                          stream: FirebaseFirestore.instance.collection("Classes").orderBy(filterChangeValue,descending: filtervalue).snapshots(),
                              ...
                              GestureDetector(
                                onTap:(){
                                  if(_formkey.currentState!.validate()){
                                    ...
                                    Navigator.pop(context);
                                    final snackBar = SnackBar(content: AwesomeSnackbarContent(title: 'Success!', message: 'Updated this Classes', contentType: ContentType.success,),);
                            ...
                            children: [
                              ...
                              GestureDetector(
                                onTap:() async {
                                  print('Justttttttttt');
                                  if(_formkey.currentState!.validate()){
                                    var document = await FirebaseFirestore.instance.collection("Classes").where("name",isEqualTo: departmentnameContoller.text).get();
                                    if(document.size <1) {
  ...
  groupfunction()async{
    var groupdocument=await FirebaseFirestore.instance.collection("Groups").where("Classes",isEqualTo:departmentnameContoller.text).get();
    Color randomColor() {
    ...
    if(groupdocument.docs.length==0){
      ...
      for(int i=0;i<AcademicYear.docs.length;i++){
        FirebaseFirestore.instance.collection("Groups").doc().set({
          ...
          "Classes":departmentnameContoller.text,
          "Img":"",
        ...
        onTap: () async {
          if (item == "Edit") {
            ...
            CoolAlert.show(
                ...
                onConfirmBtnTap:
                    () async {
                  FirebaseFirestore.instance.collection("Classes").doc(DocumentID).delete();
                  //   snackbar
                  final snackBar = SnackBar(content: AwesomeSnackbarContent(title: 'Sucess!', message: 'Classes: ${DataName} has been Deleted Sucessfully...', contentType: ContentType.success,),);
                  ScaffoldMessenger.of(context)..hideCurrentSnackBar()..showSnackBar(snackBar);
