...
class Department_Screen extends StatefulWidget {
  const Department_Screen({super.key});

  @override
  State<Department_Screen> createState() => _Department_ScreenState();
}

class _Department_ScreenState extends State<Department_Screen> {
  ...
  doclength() async {
    ...
    final QuerySnapshot result = await FirebaseFirestore.instance.collection("Department").get();
                ...
                child: SizedBox(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: [
                        StreamBuilder(
                          stream: FirebaseFirestore.instance.collection("Department").orderBy(filterChangeValue,descending: filtervalue).snapshots(),
    ...
    if(groupdocument.docs.length==0){
      ...
      for(int i=0;i<AcademicYear.docs.length;i++){
        FirebaseFirestore.instance.collection("Groups").doc().set({
          ...
          "Department":departmentnameContoller.text,
          "Img":"",
